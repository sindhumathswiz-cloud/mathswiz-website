  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <header className="mb-5">
        <h1 className="text-2xl font-black text-slate-900">Figures</h1>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1">
          <button onClick={() => setFilter('unmatched')} className={`rounded-lg px-3 py-1.5 text-sm font-black ${filter === 'unmatched' ? 'bg-indigo-600 text-white' : 'text-slate-600'}`}>Unmatched</button>
          <button onClick={() => setFilter('all')} className={`rounded-lg px-3 py-1.5 text-sm font-black ${filter === 'all' ? 'bg-indigo-600 text-white' : 'text-slate-600'}`}>All</button>
        </div>
        <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700">
          Pages
          <input inputMode="numeric" placeholder="start" value={pageStart} onChange={(e) => setPageStart(e.target.value)} className="w-16 rounded-lg border border-slate-200 px-2 py-1 text-sm" />
          <span className="text-slate-400">–</span>
          <input inputMode="numeric" placeholder="end" value={pageEnd} onChange={(e) => setPageEnd(e.target.value)} className="w-16 rounded-lg border border-slate-200 px-2 py-1 text-sm" />
        </label>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {figures.map((f) => {
            ...
            return (
              <article key={f.id} className={`rounded-2xl border bg-white p-3 ${f.questionId ? 'border-emerald-200' : 'border-slate-200'}`}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={f.imageUrl} alt={`Figure on page ${f.pageNumber}`} className="mb-2 h-40 w-full rounded-lg border border-slate-100 object-contain bg-slate-50" />
                <div className="mb-2 flex items-center justify-between text-xs font-bold text-slate-500">
                  <span>Page {f.pageNumber} · {f.imageType}</span>
                  {f.questionId ? (
                    <span className="inline-flex items-center gap-1 text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" /> {f.matchedAutomatically ? 'auto-matched' : 'manually set'}</span>
                  ) : (
                ...
                {f.questionId && f.question ? (
                  <div className="mb-2 rounded-lg bg-slate-50 p-2 text-xs text-slate-700">
                  ...
                  {f.questionId ? (
                    ...
                    <>
                      ...
                      <button onClick={() => assign(f.id)} disabled={busy || !picked[f.id]}
                        className="inline-flex items-center gap-1 rounded-lg border border-indigo-300 bg-indigo-50 px-2.5 py-1.5 text-xs font-black text-indigo-700 disabled:opacity-50">
                      ...
                      <button onClick={() => void patchFigure(f.id, { reviewed: true })} disabled={busy}
                        className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-bold text-slate-500 hover:bg-slate-50 disabled:opacity-60">
                        Not useful
                      </button>
                    </>
                  )}
