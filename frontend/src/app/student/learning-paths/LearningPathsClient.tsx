...
function StageTracker({ stage }: { stage: Path['stage'] }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {STAGES.map((s, i) => (
        <div key={s} className={`h-1.5 flex-1 rounded-full ${i <= currentIndex ? (stage === 'COMPLETED' ? 'bg-emerald-500' : 'bg-indigo-500') : 'bg-slate-100'}`} />
      ))}
    </div>
  );
}
    ...
    <main className="min-h-screen bg-slate-50 p-6 md:p-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900">Learning Paths</h1>
            <p className="text-slate-600">Examples → guided practice → timed quiz → recovery, one topic at a time.</p>
          </div>
        </div>
            ...
            {paths.map((path) => (
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-lg font-black text-slate-900">{path.topic}</p>
                  {path.stage === 'COMPLETED' ? <CheckCircle2 className="h-5 w-5 text-emerald-500" /> : path.stage === 'RECOVERY_PRACTICE' ? <RotateCcw className="h-5 w-5 text-amber-500" /> : path.stage === 'EXAMPLES' ? <BookOpen className="h-5 w-5 text-slate-400" /> : <Target className="h-5 w-5 text-indigo-500" />}
                </div>
                <StageTracker stage={path.stage} />
                <p className="mt-2 text-xs font-black uppercase tracking-wide text-slate-500">{STAGE_LABELS[path.stage]}</p>
                {path.quizScore !== null && <p className="mt-1 text-xs font-bold text-slate-400">Quiz score: {path.quizScore}%</p>}
              </Link>
            ))}
          </div>
        )}
