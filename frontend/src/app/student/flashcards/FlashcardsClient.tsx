      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Layers className="h-9 w-9 text-indigo-600" />
            <div>
              <h1 className="text-3xl font-black text-slate-900">Flashcards</h1>
              <p className="text-slate-600">Your own quick-recall cards — formulas, definitions, anything.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {dueCount > 0 && (
        ...
        {showForm && (
          <div className="mb-6 space-y-3 rounded-2xl border bg-white p-5">
            <div className="flex items-center justify-between">
              <p className="text-xs font-black uppercase tracking-wide text-slate-500">New flashcard</p>
              <button type="button" onClick={() => setShowForm(false)} className="text-slate-400 hover:text-slate-600"><X className="h-4 w-4" /></button>
            </div>
            <textarea
              value={front}
              ...
              rows={2}
              className="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold outline-none focus:border-indigo-400"
            />
            <textarea
              value={back}
            ...
            <button
              ...
              className="rounded-xl bg-indigo-600 px-5 py-2 text-xs font-black text-white disabled:opacity-40"
            >
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {cards.map((card) => (
              <div key={card.id} className="rounded-2xl border bg-white p-5">
                {card.topic && <span className="mb-2 inline-block rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black uppercase tracking-wide text-slate-500">{card.topic}</span>}
                <p className="mb-2 text-sm font-bold text-slate-900">{card.front}</p>
                <p className="mb-3 text-sm text-slate-500">{card.back}</p>
                <button
                  type="button"
