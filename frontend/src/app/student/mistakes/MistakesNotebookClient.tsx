      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <BookX className="h-9 w-9 text-indigo-600" />
            <div>
              <h1 className="text-3xl font-black text-slate-900">My Mistakes</h1>
              <p className="text-slate-600">Questions you got wrong, and questions you pinned for review.</p>
            </div>
          </div>
          {dueCount > 0 && (
            <Link href="/student/practice?mode=mistakes" className="inline-flex items-center gap-2 rounded-2xl bg-amber-500 px-5 py-3 text-sm font-black text-white shadow-lg shadow-amber-200 hover:bg-amber-600 transition-colors">
              ...
              <div key={entry.questionId} data-testid={`mistake-${entry.questionId}`} className="rounded-2xl border bg-white p-5">
                <div className="mb-2 flex flex-wrap items-center gap-2 text-xs font-black uppercase tracking-wide">
                  ...
                  {(entry.source === 'flagged' || entry.source === 'both') && (
                    <span className="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2.5 py-1 text-indigo-700"><Pin className="h-3 w-3" />pinned</span>
                  )}
                  {entry.question.topic && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-500">{entry.question.topic}</span>}
                </div>
                <div className="mb-3 text-sm text-slate-800"><MathRenderer content={entry.question.content} /></div>
                {entry.note && <p className="mb-3 text-xs italic text-slate-500">"{entry.note}"</p>}
                <div className="flex items-center gap-3">
                  <Link href={`/student/practice?mode=mistakes${entry.question.topic ? `&topic=${encodeURIComponent(entry.question.topic)}` : ''}`} className="text-xs font-black text-indigo-700 hover:underline">
                    Practice this →
                  </Link>
                  {entry.flaggedEntryId && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs font-bold text-rose-600 hover:underline disabled:opacity-50"
                    >
