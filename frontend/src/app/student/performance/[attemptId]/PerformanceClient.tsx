...
export default function PerformanceClient({ 
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-background py-8 px-4 font-sans text-gray-900 dark:text-foreground">
            <div className="max-w-6xl mx-auto space-y-8">

                {/* Header Navbar */}
                <div className="flex items-center gap-4">
                    <Link href="/student/dashboard" className="p-2 border border-gray-200 dark:border-white/10 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition bg-white dark:bg-surface shadow-sm">
                        <ChevronLeft className="w-6 h-6 text-gray-600 dark:text-slate-400" />
                    </Link>
                    <div>
                        <h1 className="font-display text-2xl font-black text-gray-900 dark:text-white">Deep Analytics Report</h1>
                        <p className="text-gray-500 dark:text-slate-400 font-medium text-sm">{attempt.test?.title} • Session recorded on {new Date(attempt.endTime).toLocaleString('en-IN')}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white dark:bg-surface rounded-3xl p-8 border border-gray-100 dark:border-white/10 shadow-sm flex flex-col">
                        <div className="mb-6">
                            <h2 className="font-display text-xl font-bold flex items-center gap-2 dark:text-white"><Clock className="w-5 h-5 text-indigo-500 dark:text-brand" /> Time Management & Behavior</h2>
                            <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Breakdown of how you spent your time across all questions.</p>
                        </div>
                        <div className="flex-1 min-h-[300px] flex items-center justify-center relative -ml-6">
                            {behavioralData.length > 0 ? (
                                ...
                                <p className="text-gray-400 dark:text-slate-500 font-bold italic">Not enough data.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-surface rounded-3xl p-8 border border-gray-100 dark:border-white/10 shadow-sm flex flex-col">
                        <div className="mb-6">
                            <h2 className="font-display text-xl font-bold flex items-center gap-2 dark:text-white"><Target className="w-5 h-5 text-purple-500 dark:text-brand-violet" /> Topic Mastery Web</h2>
                            <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Your accuracy footprint mapped across all tested subjects.</p>
                        </div>
                        <div className="flex-1 min-h-[300px] flex items-center justify-center relative">
                            {radarData.length > 0 ? (
                                ...
                                <p className="text-gray-400 dark:text-slate-500 font-bold italic">No topic tags available.</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-surface rounded-3xl p-8 border border-gray-100 dark:border-white/10 shadow-sm">
                    <div className="mb-8">
                        <h2 className="font-display text-2xl font-black mb-2 flex items-center gap-2 dark:text-white">Question Review</h2>
                        <p className="text-gray-500 dark:text-slate-400 font-medium">Step-by-step breakdown of your performance.</p>
                    </div>

                    <div className="space-y-6 relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 dark:before:via-white/10 before:to-transparent">
                        {attempt.responses.map((r: any, idx: number) => {
                            const isCorrect = r.isCorrect;
                            const isSkipped = r.status === 'SKIPPED';
                            const Icon = isSkipped ? MinusCircle : (isCorrect ? CheckCircle : XCircle);
                            const colorClass = isSkipped ? 'text-gray-400 bg-gray-100 dark:text-slate-500 dark:bg-white/5' : (isCorrect ? 'text-emerald-500 bg-emerald-100 dark:text-emerald-400 dark:bg-emerald-500/10' : 'text-red-500 bg-red-100 dark:text-rose-400 dark:bg-rose-500/10');
                            const borderClass = isSkipped ? 'border-gray-200 dark:border-white/10' : (isCorrect ? 'border-emerald-200 shadow-emerald-500/10 dark:border-emerald-500/20' : 'border-red-200 shadow-red-500/10 dark:border-rose-500/20');
                            ...
                            return (
                                <div key={r.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                    ...
                                    {/* Card */}
                                    <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] p-6 rounded-2xl border bg-white dark:bg-surface shadow-sm transition-all hover:shadow-md ${borderClass}`}>
                                        <div className="flex justify-between items-start mb-4">
                                            <span className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">Q {idx + 1}</span>
                                            <span className="text-xs font-bold text-gray-500 dark:text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {formatTime(r.timeSpent)}</span>
                                        </div>

                                        <div className="mb-6 font-medium text-gray-800 dark:text-slate-300 text-sm overflow-x-auto custom-scrollbar">
                                            <MathRenderer content={r.question?.content || 'Question content missing'} />
                                        </div>

                                        {!isSkipped && (
                                            <div className="flex flex-col gap-2 mb-6">
                                                <div className="flex items-start gap-2 bg-gray-50 dark:bg-white/5 p-3 rounded-xl border border-gray-100 dark:border-white/10">
                                                    <span className="text-[10px] font-black uppercase text-gray-400 dark:text-slate-500 tracking-wider mt-0.5 w-16 shrink-0">Your Ans</span>
                                                    <span className={`font-bold text-sm ${isCorrect ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-rose-400'}`}>{r.selectedOption || 'N/A'}</span>
                                                </div>
                                                {!isCorrect && (
                                                    <div className="flex items-start gap-2 bg-emerald-50 dark:bg-emerald-500/10 p-3 rounded-xl border border-emerald-100 dark:border-emerald-500/20">
                                                        <span className="text-[10px] font-black uppercase text-emerald-600/60 dark:text-emerald-400/60 tracking-wider mt-0.5 w-16 shrink-0">Correct</span>
                                                        <span className="font-bold text-sm text-emerald-700 dark:text-emerald-300">{r.question?.correctAnswer || 'N/A'}</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {r.question?.solution_latex && (
                                            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/10">
                                                <p className="text-xs font-bold text-indigo-500 dark:text-brand uppercase tracking-widest mb-2">Solution</p>
