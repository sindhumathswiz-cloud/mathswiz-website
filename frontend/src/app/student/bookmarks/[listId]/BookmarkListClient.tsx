...
export default function BookmarkListClient({ listId }: { listId: string }) {
  ...
  if (notFound) return <main className="min-h-screen bg-slate-50 p-10"><div className="mx-auto max-w-4xl rounded-3xl border border-dashed bg-white p-16 text-center font-bold text-slate-600">List not found.</div></main>;

  return (
    <main className="min-h-screen bg-slate-50 p-6 md:p-10">
      <div className="mx-auto max-w-4xl">
        <Link href="/student/bookmarks" className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-indigo-700">
          <ArrowLeft className="h-4 w-4" />All bookmark lists
        </Link>
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-900">{list?.name}</h1>
          {list?.description && <p className="mt-1 text-slate-600">{list.description}</p>}
        </div>
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.id} className="rounded-2xl border bg-white p-5">
                ...
                <div className="mb-3 text-sm text-slate-800"><MathRenderer content={item.question.content} /></div>
                {item.note && <p className="mb-3 text-xs italic text-slate-500">"{item.note}"</p>}
                <button
                  type="button"
