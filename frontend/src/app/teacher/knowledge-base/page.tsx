...
export default function KnowledgeBasePage() {
    ...
    const fetchFolderDetail = async (id: string) => {
        try {
            const res = await fetch(`/api/teacher/knowledge/folders/${id}`);
            const data = await res.json();
        ...
        try {
            const res = await fetch('/api/student/practice/generate', {
                ...
                body: JSON.stringify({
                    ...
                    type: genConfig.types[0],
                    difficulty: genConfig.difficulties[0],
            ...
            const data = await res.json();
            if (data.success) {
                toast.success(`Generated and Auto-Approved ${data.generated} questions!`, { id: loadingToast });
                                        ...
                                        <div className="space-y-4">
                                            <div className="relative">
                                                <input 
                                                    ...
                                                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 pr-12 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-400 transition"
                                                />
                                                <button 
                                            ...
                                            <div className="relative">
                                                <input 
                                                    ...
                                                    className="w-full bg-rose-50 border border-rose-100 rounded-2xl p-4 pr-12 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-400 transition"
                                                />
                                                <button 
                                                    onClick={() => handleUrlTrain('YOUTUBE')}
                                                    className="absolute right-2 top-2 p-2 bg-white rounded-lg text-rose-600 shadow-sm hover:scale-105 active:scale-95 transition"
                                                >
                                                    <Zap className="w-4 h-4" />
                                                </button>
                                            </div>
